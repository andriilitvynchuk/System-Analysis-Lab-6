...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prob-file", "-p", default="../data/pv_variant_3_probabilities.txt", type=str)
    parser.add_argument(
        "--output-file", "-o", default="../data/pv_variant_3_cond_probabilities.txt", type=str,
    )
        ...
        for j in range(cond_probs.shape[1]):
            if i != j:
                ...
                if upper_boundary < 0 or lower_boundary > 1:
                    raise ValueError(f"Something wrong with this range: [{lower_boundary} , {upper_boundary}]")
